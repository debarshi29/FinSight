...
Shape::
    ...
    START ─▶ plan ─▶ (Send × N) retrieve_analyze ─┬─▶ audit ────┐
                                                  └─▶ compare ──┴─▶ synthesize ─▶ END
...
SSE endpoint can surface ``retrieved`` / ``analyzed`` / ``audited`` / ``compared``
events without the graph knowing about HTTP. This layer imports agents and core
only — never ``api`` — so latency accounting is done by the caller around
``astream`` / ``ainvoke``.
"""
...
async def plan_node(state: FinSightState) -> dict:
    subtasks = await plan_task(state["query"], streaming=state.get("streaming", False))
