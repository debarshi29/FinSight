...
FinSight is orchestrated by a \textbf{LangGraph} \code{StateGraph} -- a single
compiled graph, built once and reused as a process-wide singleton by
\file{orchestration/graph.py}\code{::get\_graph()}. This replaced a Semantic Kernel
dispatch layer (documented as \code{docs/DECISIONS.md} Decision~11: the pipeline shape
never varies with the query, only the fan-out width does, which is exactly what
LangGraph's \code{Send} API expresses without a hand-rolled \code{asyncio.gather} and
manual join).

The graph has five nodes:

\begin{itemize}
    \item \code{plan} -- calls the PlannerAgent (\code{agents/router.py::plan\_task}).
    \item \code{retrieve\_analyze} -- one instance per subtask, dispatched via
    \code{Send()}, running RetrieverAgent then AnalystAgent for that subtask.
    \item \code{audit} -- calls AuditorAgent over every extracted claim in one batch.
    \item \code{compare} -- calls ComparatorAgent over the unit-normalized subtask
    results; runs in the same superstep as \code{audit}, since neither depends on the
    other.
    \item \code{synthesize} -- calls SynthesizerAgent once both \code{audit} and
    \code{compare} have completed.
\end{itemize}

Retriever, Analyst, Auditor, and Comparator are plain \code{async} functions called
directly from their nodes -- there is no dispatch layer between a node and the agent
function it calls. Planner and Synthesizer remain prompt-only roles: they call
\code{core/groq\_client.py}\code{::chat\_completion()} /
\code{chat\_completion\_hedged()} directly, which keeps those two calls on the
retry/reserve/hedge path.
...
\node[box, below=of query] (planner) {PlannerAgent\\\footnotesize plan node};
...
\end{tikzpicture}
\caption{The six-stage agent pipeline, expressed as a LangGraph \code{StateGraph}.
Auditor and Comparator run concurrently as one superstep on both the blocking and
streaming routes, since neither depends on the other's output -- only on the shared
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{ComparatorAgent} (graph superstep, concurrent with the Auditor): performs
...
Citations and claims are threaded through the pipeline as typed fields on
\code{orchestration/graph.py}\code{::FinSightState} (a \code{TypedDict}), never
serialized to JSON and re-parsed between stages. Fan-out fields
(\code{subtask\_results}, \code{errors}) use \code{Annotated[list[...],
operator.add]} reducers so LangGraph merges the parallel \code{retrieve\_analyze}
branches automatically:

\begin{center}
\ttfamily\small
user\_task $\rightarrow$ subtasks (list[str]) $\rightarrow$ subtask\_results
(reducer-merged) $\rightarrow$ verified/uncertain/unverifiable + comparison
$\rightarrow$ final report
\end{center}

\section{Concurrency and Latency Management}

Both the blocking and streaming routes drive the same compiled graph through
\file{orchestration/runner.py}\code{::run\_pipeline()}, which applies several
latency-reduction strategies inherent to the graph's structure:

\begin{itemize}
    \item All subtasks' Retriever + Analyst calls run in parallel via LangGraph's
    \code{Send} fan-out.
    \item Auditor and Comparator run concurrently as one superstep rather than
    sequentially, on \emph{both} routes now (previously stream-only), since neither
...
\section{Identity and Memory}
\label{sec:identity-memory}

Two additional best-effort nodes bracket the pipeline: \code{recall\_memory}
(before \code{plan}) and \code{remember} (after \code{synthesize}), backed by
\file{memory/store.py}\code{::MemoryService}. A memory outage degrades to no
recalled context / no write, never a failed query -- see
\file{docs/DECISIONS.md} Decision~12 for the full reasoning.

\textbf{Identity.} \file{api/middleware/auth.py}\code{::AuthMiddleware} (pure
ASGI, header-only) resolves an \code{Authorization: Bearer <key>} header to a
\code{user\_id} via \code{settings.api\_keys} (\code{"key:user\_id,..."}).
Unconfigured (the default), every request is \code{user\_id="anonymous"} --
local dev, the eval harness, and CI need no setup. This is identity for
scoping memory, not an account system: no RBAC, one flat key\,\textrightarrow\,id
mapping.

\textbf{Storage.} One new Qdrant collection, \code{finsight\_memory}, holding
\code{MemoryRecord} points: \code{memory\_id, user\_id, session\_id, task\_id,
text, timestamp}. \file{retrieval/qdrant\_store.py}\code{::QdrantStore} gained
two additive constructor parameters (\code{collection}, \code{vector\_size})
rather than a second client class, so every existing retrieval call site is
unaffected. One record is written per completed turn -- there is no separate
LLM-based ``consolidation'' step, deliberately: a compliance system's memory
should never be a new synthesis of what happened, only a record of it.

\textbf{Recall} draws on the same records two different ways:

\begin{itemize}
    \item \textbf{Short-term} (this session): filter by \code{session\_id},
    rank by recency, limit \code{settings.session\_max\_turns} (6).
    \item \textbf{Long-term} (this user, across sessions): filter by
    \code{user\_id}, rank by vector similarity to the current query, limit
    \code{settings.long\_term\_top\_k} (3).
\end{itemize}

\textbf{Where memory can and cannot reach.}
\file{memory/consolidate.py}\code{::format\_memory\_context} renders both into
one text block, injected only into \code{PLANNER\_PROMPT} (via
\code{agents/router.py}\code{::plan\_task}'s \code{memory\_context}
parameter). \code{SYNTHESIZER\_PROMPT} is untouched. This is the load-bearing
constraint: memory can bias what the Planner searches for, but it can never
become a claim, because it never enters the claims/citations path the Auditor
and Synthesizer operate on.

\textbf{Transparency.} \code{AuditLog} now carries \code{user\_id} and
\code{session\_id}. \code{GET /sessions/\{id\}} and \code{GET /memory} let a
caller inspect exactly what is recalled about them (their own records only --
a 404, not silent filtering, on someone else's session); \code{DELETE} on
both clears it.
...
FinSight/
|-- orchestration/              LangGraph StateGraph orchestration
|   |-- graph.py                 FinSightState, nodes, Send fan-out, compiled singleton
|   `-- runner.py                run_pipeline() - astream() driver, latency accounting
|-- memory/                     Per-user session + cross-session memory
|   |-- store.py                 MemoryService - session recall/write, long-term recall
|   `-- consolidate.py           build_turn_text(), format_memory_context() - no LLM call
|-- agents/                    Six-stage agent pipeline (graph nodes call these directly)
|   |-- router.py               PlannerAgent - plan_task()
|   |-- retriever.py            RetrieverAgent - RetrievalService, retrieve_chunks()
|   |-- analyst.py              AnalystAgent - analyze_chunks()
|   |-- comparator.py           ComparatorAgent - compare_results()
|   |-- auditor.py              AuditorAgent - audit_claims()
...
|   |-- middleware/auth.py      API key -> request.state.user_id (pure ASGI)
...
Orchestration & \code{langgraph} & StateGraph orchestration: Send fan-out, typed state, custom streaming \\
...
Called once, from \code{orchestration/graph.py}\code{::plan\_node}. Builds
\code{PLANNER\_PROMPT.format(user\_task=...)} and calls
\code{chat\_completion()} (or \code{chat\_completion\_hedged()} when
\code{streaming=True}, taken from graph state). Parses the LLM's JSON list output; on
...
\subsection{\file{retriever.py} --- RetrieverAgent}
\label{sec:retriever}

\code{RetrievalService} is a framework-agnostic class (no dispatch-layer decorators)
owning the Qdrant handle and the process-lifetime BM25 corpus cache; the process-wide
singleton is returned by \code{get\_retrieval\_service()} and used directly by the
graph:

\begin{lstlisting}[language=Python]
async def retrieve(self, subtask, company_filter="", fiscal_year_filter="") -> list[RankedChunk]:
\end{lstlisting}
...
\code{retrieve\_chunks()}, called from \code{orchestration/graph.py}\code{::retrieve\_analyze\_node}, which:
...
\subsection{\file{analyst.py} --- AnalystAgent}

\begin{lstlisting}[language=Python]
async def analyze_chunks(subtask, chunks_data) -> dict:  # {"kpis": [...], "claims": [...]}
\end{lstlisting}

Called directly from \code{orchestration/graph.py}\code{::retrieve\_analyze\_node} with
the chunk list already deserialized -- no JSON round-trip. It builds a context string from up to
...
\subsection{\file{auditor.py} --- AuditorAgent}
\label{sec:auditor}

\begin{lstlisting}[language=Python]
async def audit_claims(claims, chunks_by_subtask, confidence_threshold=None,
                        original_query="") -> tuple[list, list, list]:
\end{lstlisting}

Called directly from \code{orchestration/graph.py}\code{::audit\_node}, which does
\emph{not} catch exceptions from this call -- an auditor failure aborts the whole
graph run rather than shipping a report with unaudited claims. Claims are split into
two groups:
...
\subsection{\file{comparator.py} --- ComparatorAgent}

\begin{lstlisting}[language=Python]
async def compare_results(subtask_results, original_query) -> dict:
\end{lstlisting}

Called directly from \code{orchestration/graph.py}\code{::compare\_node}, after
\code{normalize\_subtask\_results()} has run; the node catches any exception from this
call and degrades to an empty comparison rather than failing a run that has verified
claims. It calls the LLM under a system prompt that
...
Called from \code{orchestration/graph.py}\code{::synthesize\_node}, which waits for
both the \code{audit} and \code{compare} nodes -- the graph's join, expressed as two
incoming edges rather than a manual \code{asyncio.gather}. A direct
\code{chat\_completion\_hedged()} call using the \code{SYNTHESIZER\_PROMPT} template
from \file{core/prompts.py}. Returns an
...
\section{Memory (\file{memory/})}
\label{sec:memory-module}

\subsection{\file{store.py} --- MemoryService}

\begin{lstlisting}[language=Python]
async def recall_session(self, session_id, limit=None) -> list[MemoryRecord]:
async def recall_long_term(self, user_id, query, top_k=None) -> list[MemoryRecord]:
async def write(self, *, user_id, session_id, task_id, text) -> MemoryRecord:
\end{lstlisting}

Wraps a \code{QdrantStore} pointed at \code{settings.memory\_collection}
(\code{finsight\_memory}), lazily \code{ensure\_collection()}-ing on first use --
the same lazy-init shape as \file{agents/retriever.py}\code{::RetrievalService}.
\code{recall\_session} is an exact \code{session\_id} filter (\code{scroll\_filtered}),
sorted by \code{timestamp}, most recent \code{settings.session\_max\_turns}.
\code{recall\_long\_term} is a \code{user\_id}-filtered dense vector search on the
embedding of the \emph{current} query. Called directly from
\code{orchestration/graph.py}\code{::recall\_memory\_node} and
\code{::remember\_node} -- no dispatch layer, same pattern as every other
capability class in this codebase.

\subsection{\file{consolidate.py}}

\begin{lstlisting}[language=Python]
def build_turn_text(query, subtasks, summary) -> str:
def format_memory_context(short_term, long_term) -> str:
\end{lstlisting}

Pure functions, no LLM call. \code{build\_turn\_text} is exactly
\code{"Query: ... \textbackslash nPlan: ... \textbackslash nSummary: ..."}
truncated to a 400-character summary excerpt -- the memory record is what the
pipeline already produced, never a new synthesis of it.
\code{format\_memory\_context} renders recalled records into the block
appended to \code{PLANNER\_PROMPT}; empty when nothing was recalled.
...
\subsection{\file{prompts.py}}

Contains the \code{PLANNER\_PROMPT} and \code{SYNTHESIZER\_PROMPT} string templates
as plain \code{str.format} strings -- no framework templating, no kernel or service
object. Imported directly by \file{agents/router.py} and \file{agents/synthesizer.py}.

\subsection{\file{orchestration/graph.py}}

Builds and holds the compiled \code{StateGraph} singleton (\code{get\_graph()}),
constructed once by \code{build\_graph()}. Defines \code{FinSightState} (the typed
pipeline state), the five nodes, the \code{Send}-based fan-out (\code{fan\_out()}),
and \code{AGENT\_SEQUENCE} (the fixed agent order recorded in the audit log, since the
parallel audit/compare superstep makes node-completion order racy).
\file{orchestration/runner.py} drives \code{graph.astream()} and derives per-agent
latency from the stream's \code{updates} events.
...
class MemoryRecord:
    ...
    text: str
    timestamp: str
    # to_payload() / from_payload() -- Qdrant point payload, not to_dict()
...
Collection \code{finsight\_memory} (\code{settings.memory\_collection}): same
384-dimensional / cosine configuration, reusing \code{retrieval/embedder.py}.
Payload fields: \code{memory\_id}, \code{user\_id}, \code{session\_id},
\code{task\_id}, \code{text}, \code{timestamp}. Short-term recall filters on
\code{session\_id} (exact match, then sorted by \code{timestamp} in Python);
long-term recall filters on \code{user\_id} and does a genuine dense vector
search against the embedded current query.
...
session\_id?\}}. Returns \code{AnalysisReport.to\_dict()} (whose
\code{audit\_log} carries the \code{session\_id} to reuse for a follow-up). \\
\code{/query/stream} & POST & \file{api/routes/query\_stream.py} & Server-Sent-Events
streaming variant, same request body; events: \code{start, memory\_recalled,
planned, retrieved, analyzed, audited, compared, remembered, done, error}. \\
...
\code{/sessions/\{id\}} & GET, DELETE & \file{api/routes/sessions.py} & The caller's
own turns for that session (404, not silent filtering, if it belongs to another
user); delete clears it. \\
\code{/memory} & GET, DELETE & \file{api/routes/memory.py} & The caller's own
long-term memory records; delete wipes them. \\
...
All routes except \code{/health}, \code{/ui*}, \code{/dashboard}, and \code{/docs}
are protected by \code{AuthMiddleware} (\code{Authorization: Bearer <key>} $\to$
\code{user\_id}, active only when \code{API\_KEYS} is configured -- see
\S\ref{sec:identity-memory}) and \code{GuardrailsMiddleware} (POST/PUT bodies
scanned for prompt-injection patterns); all routes sit behind
\code{CORSMiddleware} (\code{allow\_origins=["*"]}). Interactive Swagger
documentation is auto-served by FastAPI at \code{/docs}.
...
\begin{center}
\ttfamily
start $\to$ memory\_recalled $\to$ planned $\to$ retrieved$^\ast$ $\to$
analyzed$^\ast$ $\to$ audited $\to$ compared $\to$ remembered $\to$ done
\quad(or \texttt{error} at any point)
\end{center}

\noindent(\textsuperscript{$\ast$}emitted once per subtask). Both routes drive
the same compiled \code{StateGraph} through
\file{orchestration/runner.py}\code{::run\_pipeline()}; the streaming route
passes an \code{on\_event} callback that forwards each custom-stream event
into the SSE response via an \code{asyncio.Queue}.

\section{Shared Route Helpers}

\file{api/routes/query.py} defines the \code{QueryRequest} Pydantic model
(\code{query}, \code{company\_filter}, \code{fiscal\_year\_filter},
\code{confidence\_threshold}, \code{session\_id}) and \code{\_save\_audit\_log()}
(writes to \code{settings.audit\_log\_dir/<task\_id>.json}), reused by both
routes. \code{user\_id} is read from \code{request.state.user\_id} (set by
\code{AuthMiddleware}), never from the request body, so a caller cannot spoof
another user's identity. Subtask parsing and audit-result deserialisation --
route-level helpers in the Semantic Kernel era, reparsing JSON strings -- no
longer exist as separate functions: \code{orchestration.graph.FinSightState}
carries typed values end to end.
...
\code{AuthMiddleware} (\file{api/middleware/auth.py}) is also pure ASGI, but
header-only -- it never buffers the request body, so it needs none of
\code{GuardrailsMiddleware}'s receive-patching. It reads
\code{Authorization: Bearer <key>} from \code{scope["headers"]} and resolves it
against \code{settings.api\_keys} (parsed as \code{"key:user\_id,..."}). Empty
\code{API\_KEYS} (the default) skips straight to \code{user\_id="anonymous"};
configured, a missing or unknown key gets a \code{401 JSONResponse} before any
handler runs, on every path except \code{/health}, \code{/ui*},
\code{/dashboard}, \code{/docs}, and \code{/openapi.json}. Registered after
\code{GuardrailsMiddleware} in \file{api/main.py} so it executes \emph{first}
(Starlette runs middleware in reverse add-order) -- a bad key is rejected
before the body is buffered and scanned.
...
\code{api\_keys} & \code{""} & Comma-separated \code{key:user\_id} pairs. Empty disables auth -- every caller is \code{user\_id="anonymous"} \\
\code{memory\_collection} & \code{finsight\_memory} & Qdrant collection for \code{MemoryRecord}s \\
\code{memory\_enabled} & \code{true} & Kill switch -- \code{false} skips \code{recall\_memory}/\code{remember} entirely \\
\code{session\_max\_turns} & \code{6} & Max short-term turns recalled per session \\
\code{long\_term\_top\_k} & \code{3} & Max long-term records recalled per query \\
...
\begin{description}
    ...
    \code{AuditStatus}, \code{MemoryRecord} round-trip, \code{AuditLog}
    \code{user\_id}/\code{session\_id} defaults.
    ...
    \item[\file{test\_graph.py}] \code{StateGraph} construction (all seven nodes
    present, including \code{recall\_memory}/\code{remember}), \code{fan\_out}
    \code{Send} shape, a stubbed end-to-end run through \code{build\_graph()},
    the auditor-abort / comparator-degrade error paths, and memory-specific
    cases: recalled context reaches \code{plan\_task} and nowhere else, a
    broken memory service degrades to no context without failing the run,
    \code{memory\_enabled=False} skips recall entirely.
    \item[\file{test\_memory.py}] \code{build\_turn\_text()} /
    \code{format\_memory\_context()} as pure functions -- no Qdrant.
    \item[\file{test\_auth.py}] \code{AuthMiddleware} -- key parsing, bearer-token
    extraction, anonymous pass-through when unconfigured, 401 on missing/bad key
    when configured, exempt-path bypass. First test in the suite to exercise ASGI
    middleware directly, via \code{httpx.ASGITransport}.
\end{description}

No integration tests exercise the live FastAPI app, Qdrant, or Groq directly within
\file{tests/} -- \file{test\_auth.py} is the one exception, wrapping a minimal
Starlette app (not the full \code{create\_app()}) purely to drive real ASGI
request/response plumbing without pulling in Qdrant/embedding-model imports.
End-to-end exercise of the live app is otherwise the evaluation harness's role,
described below.
...
\file{evaluation/results/} accumulates timestamped run artifacts (git-ignored). A
representative happy-path run reports a \textbf{4/4 pass rate (1.0)}, with
per-test latency ranging roughly 8--30\,s -- e.g. the cross-company
revenue-growth comparison query returns close to 20 verified claims. The
project README states currently published scores of \textbf{4/4 on the
happy-path set and 4/4 on the adversarial set}; the previous 3/4 was a timeout
on a five-company cross-domain query, resolved once the audit/compare
superstep (\S\ref{sec:identity-memory}'s parent chapter) removed the
sequential-latency cause. Re-verified against this identity/memory change:
still 4/4 / 4/4, since recalled memory only ever reaches the Planner prompt
and cannot affect claim verification.
...
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{LangGraph replaced Semantic Kernel} (\file{docs/DECISIONS.md}
    Decision~11, 2026-09-09). SK's dispatch layer -- \code{kernel.invoke()} at every
    hop, citations threaded as JSON strings inside \code{KernelArguments} -- was
    reversed once eighteen months of living with the code showed the plan's shape
    never actually varies: plan $\rightarrow$ (retrieve + analyse per subtask,
    parallel) $\rightarrow$ audit $\parallel$ compare $\rightarrow$ synthesise is a
    fixed topology with a dynamic fan-out width, which is exactly what LangGraph's
    \code{Send} API is for. Retriever, Analyst, Auditor, and Comparator are now plain
    \code{async} functions called directly from graph nodes; Planner and Synthesizer
    remain prompt-only roles dispatched via \code{chat\_completion()} /
    \code{chat\_completion\_hedged()}, unchanged by the migration.
    \item \textbf{Auditor/Comparator concurrency is now structural, not an
    implementation-level optimization layered on top of a sequential diagram.} They
    are one LangGraph superstep -- two nodes with the same incoming edge and no edge
    between them -- so the concurrency holds on \emph{both} the blocking and streaming
    routes, not only the streaming route as before the LangGraph migration.
    \item \textbf{Per-user identity and short/long-term memory} (\file{docs/DECISIONS.md}
    Decision~12, 2026-09-09). Simple \code{API\_KEYS} $\to$ \code{user\_id}
    mapping (not full accounts/RBAC) scopes a new Qdrant collection,
    \code{finsight\_memory}, recalled by two best-effort graph nodes bracketing
    the pipeline. The load-bearing constraint: recalled memory reaches
    \code{PLANNER\_PROMPT} only, never \code{SYNTHESIZER\_PROMPT} -- it can
    bias what gets searched for, never what gets asserted as a claim, so a
    user's own unreviewed history can never become an unverified figure in the
    report. No LLM-based memory consolidation step exists, deliberately: each
    \code{MemoryRecord} is exactly what the pipeline already produced (query +
    plan + summary excerpt), traceable to a real \code{task\_id}, never a new
    synthesis of it.
