...
| [ARCHITECTURE.md](ARCHITECTURE.md) | Narrative deep-dive on the design (Semantic Kernel role, two-stage retrieval, RRF, confidence model, AuditorAgent, ingestion) | Read a longer-form explanation of one subsystem |
...
FinSight is a multi-agent RAG system for financial filings built around one constraint: every user-visible figure must resolve to a verbatim source passage, and any claim that can't be verified is blocked before synthesis rather than flagged after. A per-query LLM plan fans out into parallel hybrid retrieval (BM25 + dense → RRF → cross-encoder rerank → 5-signal confidence); an Analyst extracts only verbatim-stated claims; a deterministic Python normalizer converts all currency/scale figures to ₹ crore; a separate Auditor pass classifies each claim VERIFIED / UNCERTAIN / UNVERIFIABLE; a Comparator lays verbatim figures side by side; a Synthesizer writes the report from verified evidence only. Every run persists a machine-readable audit log.
